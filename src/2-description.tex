...
# => ((f(x) >= g(x, y)) ? f(x) : g(x, y))
\end{lstlisting}
Comparative expression are stored in special container classes, modeled by the ancestor \CASExpression.
